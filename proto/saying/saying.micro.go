...
func NewSayingService(name string, c client.Client) SayingService {
	if c == nil {
		c = client.NewClient()
	}
	if len(name) == 0 {
...
func RegisterSayingHandler(s server.Server, hdlr SayingHandler, opts ...server.HandlerOption) error {
	...
	h := &sayingHandler{hdlr}
	return s.Handle(s.NewHandler(&Saying{h}, opts...))
}

type sayingHandler struct {
	SayingHandler
}
